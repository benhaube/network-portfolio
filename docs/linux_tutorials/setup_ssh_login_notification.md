...
2.  Paste the following into the configuration file:
    ...
    ``` bash { .mono-title title="/etc/msmtprc" linenums="1" hl_lines="11 13-14" }
    --8<-- "msmtprc"
    ```
...
1.  Edit `/etc/pam.d/sshd` and add the following **after** the existing "session" lines:
    ...
    1. Add line to `/etc/pam.d/sshd` _after_ the existing "session" lines.

    !!! warning  

        It is important to use `#!bash sudo -e` instead of a direct editor command _(like `sudo nano`)_ when editing system configuration files. This ensures the file is checked for errors before it is saved, using the editor specified by your system's `$EDITOR` environment variable.

    The final file should look like this:

    ``` desktop { .mono-title title="/etc/pam.d/sshd" linenums="1" hl_lines="26" }
    --8<-- "pam-sshd"
    ```
...
2.  Paste the following into your script file, and replace `example@example.com` with the email address to which you would like the notifications to be sent.
    ...
    ``` bash { .mono-title title="ssh-login-notify.sh" linenums="1" hl_lines="7" }
...
3.  Paste the following code into your script file, and replace the variables `TOKEN` and `URL` with your actual Gotify App token and URL.
    ...
    ``` bash { .mono-title title="gotify-ssh-alert.sh" linenums="1" hl_lines="9 10" }
...
???+ tip
    ...
    2. Find _(or add)_ the `Sender` line and set it to your authorized email:
