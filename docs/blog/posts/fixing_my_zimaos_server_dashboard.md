...
This command will output the data URI which I could paste directly into the icon URL field in my ZimaOS dashboard. I could simply type this command every time I want to set or change an icon for a service on the dashboard, but that would become pretty tedious rather quickly. Instead I decided to make a special function. I called my function `svg2uri`. Pretty simple right? It is a LOT faster than typing that super long command every time, and it is easier than having to paste that command from a text document too.
...
``` bash { .mono-title title="svg2uri" linenums="1" }
svg2uri() {
...
If you have a NAS running ZimaOS, or you just want to convert SVG files for easy use in HTML or CSS files you can check out the step-by-step tutorial _(linked in the sidebar)_ I have written to set this up for yourself.
