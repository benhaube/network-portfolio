...
1.  **Identify your LUKS device:**
	+ Find the partition or block device that contains your LUKS volume using `lsblk` or `fdisk -l`.
    	+ _Example:_ `/dev/nvme0n1p3`
...
3.  **Configure `crypttab`:**
	...
	+ Find the line for your LUKS volume and append `tpm2-device=auto` to the options field _(the fourth column)_.
...
4.  **Configure Grub:**
    ...
    + Open `/etc/default/grub` with a text editor as a superuser. _(e.g., using `nano` or `vim`)_
