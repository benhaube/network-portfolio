!!! config inline "Critical Configurations"
    ...
    **:symbols-clock-refresh-cw:&ensp;Chrony:**
    ...
    :    Do not modify NTP settings or enable `systemd-timesyncd`. It has been replaced with [Chrony](../docs/03_services/chrony.md). The Chrony service is hosted on the main router, [ASUS RT-BE92U](../docs/02_hardware/asus_rt-be92u.md).
    ...
    **:symbols-mail-warning:&ensp;Email Notifications:**
    ...
    :    This server has email & push notifications configured for new SSH sessions and `unattended-upgrades`. See [Setup SSH Login Notification](../docs/linux_tutorials/setup_ssh_login_notification.md) for documentation.
...
+ Updates for services using a Docker image can be applied manually with `docker compose` or by using the update utility in [Dockge](../docs/03_services/dockge.md)

#### :symbols-cloud-upload:&ensp;Backup Policy

+ An image of the MicroSD card is stored on [ZimaOS NAS](../docs/02_hardware/zimaos_nas.md). 
    + Path: `/media/Quick-Storage/Backup/<hostname>`
+ Data and configurations from Docker services are backed up to [ZimaOS NAS](../docs/02_hardware/zimaos_nas.md). 
...
+ All backed up files stored on the **ZimaOS NAS** are then backed up to the cloud storage provider, [Backblaze B2:symbols-external-link-small:](https://www.backblaze.com/cloud-storage), to maintain the [3-2-1 Backup Strategy](../docs/01_infrastructure/disaster_recovery_plan.md#backup-strategy).
